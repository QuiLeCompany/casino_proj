 ...
 */
    ...
    public static isIpad(windowSize: math.Size): boolean {
        // because portrait mode
        let isIpad = false;
        let IPAD_W = 2048;
        let IPAD_H = 2732;
        let size = windowSize;
        // make sure alway correct
        console.log(`@@@@@@@@@@@@@ isIPAD check
            size.height = ${size.height}
            size.width = ${size.width}
        `);
        if (size.height > size.width ? size.height / size.width <= IPAD_H / IPAD_W : size.width / size.height <= IPAD_H / IPAD_W) {
            isIpad = true;
        }
        return isIpad;
    }
