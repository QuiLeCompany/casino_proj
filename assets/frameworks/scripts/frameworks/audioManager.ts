...
export class AudioManager {
     ...
     */
    playMusic(name: string, loop: boolean, stopBeforePlay = false) {
        MahjongSingleton.IS_DEBUG_FROM_SND && console.error(`playMusic : ${name} trace: ${new Error().stack}`);
        ...
        resourceUtil.loadRes(path, AudioClip, (err: any, clip: any) => {
            let source = self._getAudioSource(clip);
            let tmp: AudioData = {
                source,
                isMusic: true,
            };
            self.audios[name] = tmp;
            if(stopBeforePlay) source.stop();
            source.volume = self.musicVolume;
            source.loop = loop;
            source.play();
        });
    }
    ...
    playSound(name: string, loop: boolean = false) {
        MahjongSingleton.IS_DEBUG_FROM_SND && console.error(`playSound : ${name} trace: ${new Error().stack}`);
        if (!this.soundVolume) {
            return;
        }
        ...
        resourceUtil.loadRes(path + name, AudioClip, (err: any, clip: any) => {
            ...
            source.node.on(AudioSource.EventType.ENDED, () => {
                loadsh.remove(self.arrSound, (obj: AudioData) => {
    ...
    loadAudioVolume() {
        this.soundVolume = StorageManager.instance.getGlobalData(`sound_volume`) ? StorageManager.instance.getGlobalData(`sound_volume`) : 1;
        this.musicVolume = StorageManager.instance.getGlobalData(`music_volume`) ? StorageManager.instance.getGlobalData(`music_volume`) : 0.8;
    }
    ...
    saveAudioVolume() {
        StorageManager.instance.setGlobalData('sound_volume', this.soundVolume);
        StorageManager.instance.setGlobalData('music_volume', this.musicVolume);
    }
